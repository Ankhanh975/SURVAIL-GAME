...
(function () {
  ...
  var CMap = (function () {
    ...
    var C = function (params) {
      ...
      this.generation._float = parseFloat(
        "0." + Math.abs(this.generation._num)
      );
    ...
    C.prototype.tick = function (timeSinceLastFrame) {
      ...
      this.xTileMax = Math.floor(
        (this.viewRect.x + this.viewRect.w) / CHUNK_SIZE
      );
      this.yTileMin = Math.floor(this.viewRect.y / CHUNK_SIZE);
      this.yTileMax = Math.floor(
        (this.viewRect.y + this.viewRect.h) / CHUNK_SIZE
      );
    };
    ...
    C.prototype.isChunkVisible = function (x, y) {
      ...
      if (
        !Game.Utils.intersectRect(
          // AABB
          {
            top: pos.r1.y,
            left: pos.r1.x,
            bottom: pos.r1.y + pos.r1.h,
            right: pos.r2.x + pos.r2.w,
          },
          {
            top: pos.r2.y,
            left: pos.r2.x,
            bottom: pos.r2.y + pos.r2.h,
            right: pos.r2.x + pos.r2.w,
          }
        )
      ) {
  ...
  var CPlayer = (function () {
    ...
    var C = function (params) {
      ...
      this.r = 0.0;
      this.x =
        typeof params.x === "undefined" || params.x === null
          ? DEFAULT_PLAYER_X
          : params.x;
      this.y =
        typeof params.y === "undefined" || params.y === null
          ? DEFAULT_PLAYER_Y
          : params.y;
      ...
      this.weapons = [new CWeapon(0), new CWeapon(1)];
    ...
    C.prototype.draw = function (timeSinceLastFrame, context) {
      ...
      Game.Utils.drawImageRot(
        ...
        this.w,
        this.h,
        Game.Utils.convertToDegree(this.r) - this.srot
      );
        ...
        if (sets.hasOwnProperty(i)) {
          ...
          if (sets[i].animations) {
            for (a in sets[i].animations) {
              ...
              ts[i].animations[a]._current =
                sets[i].animations[a].frames[0] || 0;
    ...
    for (i; i < l; i++) {
      ...
      if (t.sub == "tree") {
        ...
        textureSet.drawObj(
          ...
          "trees",
          "palm",
          pos.px + TILE_SIZE * t.x,
          pos.py + TILE_SIZE * t.y,
          TILE_SIZE,
          TILE_SIZE
        );
      }
  ...
  function CreateScene(engine, params, callback) {
    ...
    var __onTileSetLoaded = function onTileSetLoaded(success) {
      if (!success) {
        console.error(
          "CreateScene()",
          "Cannot continue -- Error loading textures!"
        );
        return;
      }
    ...
    var __onAudioLoaded = function onAudioLoaded(success) {
      if (!success) {
        console.error(
          "CreateScene()",
          "Cannot continue -- Error loading audio!"
        );
        return;
      }
  ...
  function RenderScene(timeSinceLastFrame, engine, input, fps) {
    ...
    engine.context.fillText(
      "2D Tile Engine (c) Anders Evenrud",
      canvasWidth - 190,
      5
    );
