...
from roguey.classes.constants import *
from roguey.classes.items import Treasure
from roguey.classes.monsters import Derpy
            ...
            if self.walls[check[0]][check[1]]:
                ...
                if self.walls[next[0]][next[1]]:
                    ...
                    try:
                        ...
                        # Sometimes, we're one away from the border. That's okay.
                        pass
        except:
            return False
        return True
    ...
    def set_current_position(self, position):
        ...
        for i in range(RADIUS-1):
            if row-i > 0 and col-i > 0:
                self.current[row-i-1][col-i-1] = 1
            if row-i > 0 and col-i < COLUMNS-1:
                self.current[row-i-1][col+i+1] = 1
            if row+i < ROWS-1 and col-i > 0:
                self.current[row+i+1][col-i-1] = 1
            if row+i < ROWS-1 and col+i < COLUMNS-1:
                self.current[row+i+1][col+i+1] = 1
        ...
        for monster in monsters.keys():
            if monster.current_hp <= 0:
                ...
                try:
                    ...
                    # Monsters can run into walls, edges, chests, etc. It consumes their turn.
                    pass
